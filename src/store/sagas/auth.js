...
// a star behind function is a generator,
// next generation js features, function that 
...
export function* authUserSaga(action) {
  ...
  if (action.method === 'login') {
    url = 'https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key=AIzaSyB3oaZxMbRIdzfp4mkoIX64qNGBlOSQmgk'
  }

  // yield is used as a 
  // next gen js which will wait a promise to resolve or 
  // reject then store the value in this variable
  try {
    ...
    yield put(actions.authSuccess(
      response.data.idToken, response.data.localId));
